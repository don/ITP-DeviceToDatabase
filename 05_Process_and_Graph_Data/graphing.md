...
Create a file name `.env` to store the TimescaleDB connection information. Replace `user`, `password` with your TimescaleDB username and password. The database `instance` can be your database or the `itp` database.
...
Use Control + C to stop the server. Source the environment file. Restart the server.
