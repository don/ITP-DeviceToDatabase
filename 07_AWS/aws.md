...
![Cloud Formation - Create Stack](img/aws-cloudformation-create-agree.png)
...
Load the [GenerateCSR.ino](../02_Arduino/arduino/GenerateCSR/GenerateCSR.ino) sketch on your Arduino Nano 33 IoT (or MRK WiFi 1010).

Open the Serial Monitor and generate a CSR. Press the "Send" button or enter key to accept all the default values. The device name is pre-populated with the serial number from the ECCX08 chip. Refer to the [Arduino chapter](../02_Arduino/exercises/exercise8.md) if you need more details.

![Screenshot of Arduino sketch generating CSR](../02_Arduino/exercises/images/generate-csr.png)

Copy the CSR from the serial monitor and save to a new text file named `arduino.csr`. Leave the serial monitor open, since you'll need the device serial number in the next step.
 ...
 * Use the **Serial Number** *(e.g. 0123CFBA6DEAD444EE)* for your device name. Scroll down and choose Next.
 * Choose `Upload CSR`
 * Use the `Choose file` button, select `arduino.txt` in the Finder window
 * Choose `Upload File`
 * Choose `Next`
 * Check the box next to `ThingPolicy` to attach the policy your device
 * Press the `Create Thing` button
 ...
 * Click on the Certificate ID to show the details
 * Use the Action menu to Activate the certificate
 * Use the Action menu to Download the certificate
 * Reanme the dowloaded file to `arduino_cert.txt`
...
AWS generates a custom host name for each Core IoT installation. Choose [Settings](https://us-east-1.console.aws.amazon.com/iot/home?region=us-east-1#/settings) from the bottom of the main AWS IoT menu to view the custom endpoint. Copy this value, you'll need it in the next step to connect your Arduino to AWS.

![Screenshot of AWS Core IoT custom endpoint](img/aws-iot-custom-endpoint.png)

## Connect

The next task is to get the Arduino connected to AWS by configuring the [AWS.ino](../02_Arduino/arduino/AWS/AWS.ino) sketch and loading it onto your Arduino. The config.h file contains all the variables you need to edit. The broker URL is the custom MQTT endpoint from the Core IoT Settings page in the last step. The device certificate is the certificate file you downloaded from AWS. Make sure you use the certificate and not the CSR. The ArduinoBearSSL library uses the certificate from config.h and private key from the crypto chip to authenticate with the AWS server in place of a username and password. Review the [instructions from the Arduino chapter](../02_Arduino/exercises/exercise8.md#awsino) if necessary.
...
NOTE: This sketch supports a dimmable LED. The LED_BUILTIN (pin 13) on the Arduino Nano 33 IoT does not support PWM. Move the LED wire from Pin 13 to Pin 12.
