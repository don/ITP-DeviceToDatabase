...
We are going to create an Express app to query the database and use Google Charts to create a graph to display information for the user.
...
Open the `dev2db-charts` directory in [Visual Studio Code](https://code.visualstudio.com). If Visual Studio Code is in your PATH, you can run `code .` to open the current directory in the editor. 
...
Start the server from a terminal to make sure everything is working
...
Next we are are going to create some functions that return JSON encoded data over HTTP.

## Get devices

Add a new route `/device` which returns a list of devices that have temperature data. Add this after `app.get('/', ...)` and before `app.listen...`.
...
Add another route to get temperature data for a specific device. The `:device` is a route parameter and will be filled in with a real device name. When clients call `/device/:device/temperature` it will look like `/device/device_01/temperature` or `/device/device_99/temperature`.
...
Now that we have APIs to serve data we need some web pages to use the APIs. The web page will load in the web browser. Javascript running in the web page will call the server to get device and temperature information.

Express can be configured to serve static files from a directory. We will use this to send HTML and Javascript pages to the web browser. Add the express static middleware to `server.js` before the first `app.get('/'` line.

    app.use(express.static('public'));

Comment out the line that starts with `app.get('/'` to  **remove** the old default route
...
Restart your server and test that your index.html file is being served from http://localhost:4000.

![screenshot of safari loading index.html page](img/hello-index.png)

## pm2-dev

Restarting the server after each change can be tedious. We can use `pm2-dev` to watch for changes to our project and automatically restart the server. Use Control + C to stop the server. If necessary install pm2, then restart the server with pm2-dev. MacOS users might need sudo when installing pm2 globally.
...
Now that our server is working, we can write the client side of our app. Replace the existing contents of `public/index.html` with the following.
...
When the selected item in the device dropdown changes, the web page needs to ask the server for temperature data from that device. Add the `onDeviceSelected` event handler to `index.js`.
...
The last line of the  `index.js` file needs to load the Google Chart package.
